...
async def get_token_history(ca, timeframe="7D"):
    gt_path = {"1H": "minute/5", "6H": "hour/1", "24H": "hour/1", "7D": "hour/4", "30D": "day/1"}
    gt_limit = {"1H": 12, "6H": 6, "24H": 24, "7D": 42, "30D": 30}
    path = gt_path.get(timeframe, "hour/4")
    lim = gt_limit.get(timeframe, 42)

    # Сначала находим адрес пула токена
    pool_address = None
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    if pools:
                        ...
                        pool_address = pools[0].get("attributes", {}).get("address") or \
                                       pools[0].get("id", "").split("_")[-1]
                        logger.info(f"Found pool for {ca[:8]}: {pool_address}")
    except Exception as e:
    ...
    # Теперь тянем OHLCV по адресу пула
    for attempt in range(2):
        try:
            async with aiohttp.ClientSession() as s:
                async with s.get(
                    f"https://api.geckoterminal.com/api/v2/networks/ton/pools/{pool_address}/ohlcv/{path}?limit={lim}",
                    timeout=aiohttp.ClientTimeout(total=10),
                    headers={"Accept": "application/json"}
                ) as r:
                    if r.status == 200:
                        ...
                        if len(candles) >= 2:
                            ...
                            logger.info(f"OHLCV: {len(history)} candles for pool {pool_address[:8]}")
                            return history
                    elif r.status == 429:
