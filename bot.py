...
async def edit_or_answer(message, text, reply_markup=None, parse_mode="HTML"):
...
async def auto_detect_ca(message: types.Message, state: FSMContext):
    ...
    if message.chat.type in ("group", "supergroup"):
        ...
        if existing_call:
            ...
            # Записываем калл после получения данных
            try:
                price, symbol, mcap, _, _, _, _, _ = await fetch_token_data(ca)
                if price:
                    record_call(message.chat.id, message.from_user.id, username, ca, symbol, price, mcap)
            except: pass
        return
...
async def send_token_info_group(chat_id, ca, edit_msg=None, caller_info=None):
    ...
    if caller_info is None:
        record_call(chat_id, 0, "unknown", ca, symbol, price, mcap)
