...
# ─── TON PRICE CACHE ──────────────────────────────────────────────────────────
_ton_price_cache = {"price": 5.0, "updated": 0}

async def get_ton_price():
    now = asyncio.get_event_loop().time()
    if now - _ton_price_cache["updated"] < 60:
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status == 200:
                    data = await r.json()
                    price = float(data.get("the-open-network", {}).get("usd", 5.0))
                    _ton_price_cache["price"] = price
                    _ton_price_cache["updated"] = now
                    return price
    except Exception as e:
        logger.warning(f"TON price: {e}")
    return _ton_price_cache["price"]
...
async def _fetch_mcap_from_pools(ca: str) -> float:
    """Получаем mcap из первого пула если fdv не пришёл напрямую"""
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    if pools:
                        ...
                        fdv = float(attrs.get("fully_diluted_valuation") or attrs.get("market_cap_usd") or 0)
                        return fdv
    except:
        pass
    return 0.0
...
async def fetch_token_data(ca: str):
    """Returns (price_usd, symbol, fdv, liquidity, image_url)"""
    # 1. GeckoTerminal — 3 попытки
                ...
                async with s.get(
                    ...
                    if r.status == 200:
                        ...
                        if price > 0:
                            ...
                            if fdv <= 0:
                                fdv = await _fetch_mcap_from_pools(ca)
                            return price, symbol, fdv, liq, image
                    elif r.status == 429:
    ...
    for attempt in range(2):
        try:
            async with aiohttp.ClientSession() as s:
                async with s.get(
                    f"https://api.ston.fi/v1/assets/{ca}",
                    ...
                    if r.status == 200:
                        ...
                        if price > 0:
                            return price, asset.get("symbol") or ca[:6].upper(), 0.0, 0.0, ""
        except Exception as e:
            ...
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    if pools:
                        ...
                        if price > 0:
                            return price, ca[:8].upper(), 0.0, 0.0, ""
    except Exception as e:
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                f"https://tonapi.io/v2/jettons/{ca}",
                timeout=aiohttp.ClientTimeout(total=10)
            ) as r:
                if r.status == 200:
                    data = await r.json()
                    ...
                    if price > 0:
                        return price, symbol, 0.0, 0.0, ""
    except Exception as e:
        logger.warning(f"TonAPI: {e}")

    return 0.0, ca[:8].upper(), 0.0, 0.0, ""
...
def generate_chart(symbol, history, image_arr=None, entry_price=None, fdv=None, current_price=None):
    # Если нет истории но есть текущая цена — делаем минимальный график
    if (not history or len(history) < 2) and current_price:
        from datetime import timedelta
        now = datetime.now()
        history = [[int((now - timedelta(hours=i)).timestamp() * 1000), current_price] for i in range(6, -1, -1)]
    ...
    try:
        ...
        ax.set_title(title, color=color, fontsize=13, fontweight="bold", pad=14)
        if image_arr is not None:
            try:
                im = OffsetImage(image_arr, zoom=0.6)
                ab = AnnotationBbox(im, (0.04, 0.90), xycoords='axes fraction', frameon=False, box_alignment=(0,1))
                ax.add_artist(ab)
            except: pass
        plt.tight_layout()
        buf = io.BytesIO()
...
def generate_pnl_card(symbol, pnl_pct, pnl_gram, mcap_buy, mcap_sell, entry_price, exit_price, image_arr=None):
    try:
        ...
        ax.text(5, 6.0, f"{arrow}  {pnl_pct:+.2f}%", color=accent, fontsize=34,
                fontweight="bold", ha="center", va="center")
        gram_sign = "+" if pnl_gram >= 0 else ""
        ax.text(5, 4.5, f"{gram_sign}{format_gram(pnl_gram)} 💎 GRAM",
                color="#cccccc", fontsize=13, ha="center", va="center")
        ...
        ax.text(3.0, 2.0, f"Entry: {format_price(entry_price)}", color="#666688", fontsize=9, ha="center")
        ax.text(7.0, 2.0, f"Exit: {format_price(exit_price)}", color="#666688", fontsize=9, ha="center")
        ...
        logger.warning(f"PnL card error: {e}")
        plt.close('all')
        return None
...
def back_keyboard(refresh_cb=None):
    kb = InlineKeyboardMarkup()
    if refresh_cb:
        kb.add(InlineKeyboardButton("🔄 Refresh", callback_data=refresh_cb))
    kb.add(InlineKeyboardButton("◀️ Back", callback_data="back_main"))
    return kb
...
async def send_token_info(chat_id, ca, state: FSMContext, edit_msg=None):
    price, symbol, fdv, liq, image_url = await fetch_token_data(ca)
    ...
    mcap_str = format_mcap(fdv) if fdv > 0 else "N/A"
    max_buy = balance / price_in_gram if price_in_gram > 0 else 0

    caption = (
        f"🪙 <b>{symbol}</b>\n\n"
        f"💵 Price:  <b>{format_price(price)}</b>  ·  <b>{format_gram(price_in_gram)} 💎</b>\n"
        f"📊 MCap:  <b>{mcap_str}</b>\n"
        f"💧 Liquidity:  <b>{liq_str}</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💎 Your balance:  <b>{format_gram(balance)} GRAM</b>\n"
        f"📦 Max buy:  <b>{format_gram(max_buy)} {symbol}</b>\n\n"
        f"🔗 <a href='{dex_link(ca)}'>View on DexScreener</a>\n\n"
        f"Enter amount in <b>💎 GRAM</b> or tap:"
    )

    # Загружаем картинку токена и генерируем график
    image_arr = load_token_image(image_url) if image_url else None
    history = await get_ton_token_history(ca, days=7)
    chart_buf = generate_chart(symbol, history, image_arr=image_arr, fdv=fdv, current_price=price)
...
async def ca_amount_entered(message: types.Message, state: FSMContext):
    ...
    await state.update_data(gram_amount=gram_amount, coin_amount=coin_after_slip, slip_pct=slip_pct, mcap=fdv)
    ...
    await message.answer(
        f"✅ <b>Confirm Buy</b>\n\n"
        f"🪙 {symbol}:  <b>{format_gram(coin_after_slip)}</b>\n"
...
async def sell_coin_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    price, _, fdv, _, image_url = await fetch_token_data(ca) if ca else (avg_price, symbol, 0, 0, "")
    ...
    safe_sym = re.sub(r'[^A-Za-z0-9]', '', symbol)[:12]
    arrow = pnl_arrow(pnl_pct)
    cur_mcap = fdv if fdv > 0 else 0

    mult_str = ""
    if avg_mcap > 0 and cur_mcap > 0:
        mult = cur_mcap / avg_mcap
        mult_str = f"\n📊 MCap:  {format_mcap(avg_mcap)} → {format_mcap(cur_mcap)}  (<b>{mult:.2f}x</b>)"

    await state.update_data(symbol=symbol, price=price, ca=ca, safe_sym=safe_sym,
                            held=held, avg_price=avg_price, avg_mcap=avg_mcap,
                            cur_mcap=cur_mcap, image_url=image_url, fdv=fdv)
...
async def confirm_sell(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"🪙 {symbol}:  -{format_gram(coin_amount)}\n"
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    if not holdings:
        await callback.message.edit_text(
            ...
            reply_markup=back_keyboard("portfolio"), parse_mode="HTML"); return

    total_value = balance
    lines = [f"💼 <b>Portfolio</b>\n\n💎 Free:  <b>{format_gram(balance)} GRAM</b>  <i>(≈ ${balance_usd:,.2f})</i>\n"]

    for sym, amount, avg_price, ca, avg_mcap in holdings:
        price, _, fdv, _, _ = await fetch_token_data(ca) if ca else (avg_price, sym, avg_mcap, 0, "")
        if not price: price = avg_price
        price_in_gram = price / ton_price
        avg_price_gram = avg_price / ton_price
        ...
        lines.append(
            ...
            f"   📊 {format_mcap(avg_mcap)} → {format_mcap(cur_mcap)}{mult_str}\n"
            f"   {'🟢' if pnl_pct>=0 else '🔴'} <b>{pnl_pct:+.2f}% {arrow}</b>  ({'+' if pnl_gram>=0 else ''}{format_gram(pnl_gram)} 💎)\n"
            f"   💎 {format_gram(value)} GRAM"
        )
...
async def show_history(callback: types.CallbackQuery):
    ...
    for sym, action, amount, price, price_buy, total, pnl_pct, mcap_buy, mcap_sell, ts in trades:
        if action == "BUY":
            lines.append(
                f"🟢 {ts}  ·  <b>{sym}</b>\n"
                f"   +{format_gram(amount)}  @  {format_price(price)}  ·  {format_gram(total)} 💎\n"
