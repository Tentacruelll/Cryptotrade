...
async def send_token_info(chat_id, ca, state: FSMContext, edit_msg=None, timeframe="7D"):
    ...
    created_str = f"\n🗓 Created:  <b>{created_at}</b>" if created_at else ""
...
async def send_token_info_group(chat_id, ca, edit_msg=None):
    ...
    text = (
        ...
        f"<code>{ca}</code>\n"
        f"🔗 <a href='{dex_link(ca)}'>DexScreener</a>\n\n"
        f"<i>Trade on @OCTOtrade_bot</i>"
    )
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("🔗 Open in OCTOtrade", url=f"https://t.me/OCTOtrade_bot?start={ca}"))
    try: await edit_msg.delete()
    except: pass
