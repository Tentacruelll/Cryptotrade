...
# ─── TON PRICE CACHE ─────────────────────────────────────────────────────────
_ton_cache = {"price": 0.0, "updated": None}

async def get_ton_price():
    now = datetime.now()
    # Используем кэш только если он свежий (< 90 сек) и цена реальная
    if (_ton_cache["updated"] and (now - _ton_cache["updated"]).total_seconds() < 90 and _ton_cache["price"] > 1.0):
    ...
    for url in sources:
        try:
            async with aiohttp.ClientSession() as s:
                async with s.get(url, timeout=aiohttp.ClientTimeout(total=6)) as r:
                    if r.status == 200:
                        data = await r.json()
                        # CoinGecko format
                        p = data.get("the-open-network", {}).get("usd", 0)
                        if not p:
                            # Coinbase format
                            p = data.get("data", {}).get("amount", 0)
                        p = float(p or 0)
                        if p > 1.0:
                            _ton_cache["price"] = p
            ...
            logger.warning(f"TON price from {url}: {e}")

    # Если все источники упали — возвращаем последнюю известную цену
    if _ton_cache["price"] > 1.0:
        logger.warning("TON price: using stale cache")
        return _ton_cache["price"]

    # Последний резерв — запрос к binance
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get("https://api.binance.com/api/v3/ticker/price?symbol=TONUSDT", timeout=aiohttp.ClientTimeout(total=6)) as r:
                if r.status == 200:
                    data = await r.json()
        ...
        logger.warning(f"Binance TON: {e}")

    logger.error("TON price: all sources failed, using 5.0 as last resort")
    return _ton_cache["price"] if _ton_cache["price"] > 0 else 5.0
...
async def fetch_token_data(ca: str):
    """Returns (price_usd, symbol, mcap, is_fdv, liquidity, image_url)"""
                ...
                async with s.get(
                    ...
                    if r.status == 200:
                        ...
                        if price > 0:
                            ...
                            if mc > 0:
                                return price, symbol, mc, False, liq, image
                            elif fdv > 0:
                                return price, symbol, fdv, True, liq, image
                            else:
                                # Пробуем pools
                                mc2, is_fdv2 = await _fetch_mcap_from_pools(ca)
                                return price, symbol, mc2, is_fdv2, liq, image
                    elif r.status == 429:
            ...
            async with aiohttp.ClientSession() as s:
                async with s.get(f"https://api.ston.fi/v1/assets/{ca}",
                    ...
                    if r.status == 200:
                        ...
                        if price > 0:
                            return price, asset.get("symbol") or ca[:6].upper(), 0.0, True, 0.0, ""
        except Exception as e:
            ...
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    if pools:
                        ...
                        mc = float(attrs.get("market_cap_usd") or attrs.get("fully_diluted_valuation") or 0)
                        if price > 0:
                            return price, ca[:8].upper(), mc, True, 0.0, ""
    except Exception as e:
        ...
        async with aiohttp.ClientSession() as s:
            async with s.get(f"https://tonapi.io/v2/jettons/{ca}",
                ...
                if r.status == 200:
                    ...
                    if price > 0:
                        return price, symbol, 0.0, True, 0.0, ""
    except Exception as e:
        logger.warning(f"TonAPI: {e}")

    return 0.0, ca[:8].upper(), 0.0, True, 0.0, ""
...
async def get_token_history(ca, timeframe="7D"):
    ...
    resolution, limit, _ = tf_map.get(timeframe, ("day", 7, 86400))
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                f"https://api.geckoterminal.com/api/v2/networks/ton/tokens/{ca}/ohlcv/{resolution}?limit={limit}",
                timeout=aiohttp.ClientTimeout(total=10),
                headers={"Accept": "application/json"}
            ) as r:
                if r.status == 200:
                    ...
                    if ohlcv:
                        return [[item[0]*1000, float(item[4])] for item in reversed(ohlcv)]
    except Exception as e:
        logger.warning(f"History {timeframe}: {e}")
    return []
...
def generate_chart(symbol, history, image_arr=None, entry_price=None, mcap=None,
    ...
    if (not history or len(history) < 2) and current_price:
        now = datetime.now()
        history = [[int((now - timedelta(hours=i)).timestamp()*1000), current_price]
...
def generate_pnl_card(symbol, pnl_pct, pnl_gram, mcap_buy, mcap_sell,
    ...
    try:
        ...
        ax.text(5, 0.8, "Tenta Trader  ·  Demo", color="#333355", fontsize=8, ha="center")
...
async def send_token_info(chat_id, ca, state: FSMContext, edit_msg=None, timeframe="7D"):
    price, symbol, mcap, is_fdv, liq, image_url = await fetch_token_data(ca)
    ...
    caption = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💎 Balance:  <b>{format_gram(balance)} GRAM</b>\n"
...
async def sell_coin_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    price, _, cur_mcap, is_fdv, _, image_url = await fetch_token_data(ca) if ca else (avg_price, symbol, 0, True, 0, "")
...
async def confirm_sell(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"{'🟢' if pnl_pct>=0 else '🔴'} P&L:  <b>{pnl_pct:+.2f}% {arrow}</b>  {x_str}\n"
        f"📊 MCap:  {format_mcap_val(avg_mcap)} → {format_mcap_val(cur_mcap)}  {mc_chg}\n"
        f"💎 {'+' if pnl_gram>=0 else ''}{format_gram(pnl_gram)} GRAM\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💎 Balance:  <b>{format_gram(user[2])} GRAM</b>  <i>(≈ ${user[2]*ton_price:,.2f})</i>",
        reply_markup=main_keyboard(), parse_mode="HTML"
    )
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    total_value = balance
    lines = [f"💼 <b>Portfolio</b>\n\n💎 Free:  <b>{format_gram(balance)} GRAM</b>  <i>(≈ ${balance*ton_price:,.2f})</i>\n"]

    for sym, amount, avg_price, ca, avg_mcap in holdings:
        price, _, cur_mcap, is_fdv, _, _ = await fetch_token_data(ca) if ca else (avg_price, sym, avg_mcap, True, 0, "")
        if not price: price = avg_price
        price_in_gram = price / ton_price
        avg_price_gram = avg_price / ton_price
        ...
        lines.append(
            ...
            f"   💎 {format_gram(value)} GRAM  ({'+' if pnl_gram>=0 else ''}{format_gram(pnl_gram)})"
        )
