                    ...
                    if r.status == 200:
                        ...
                        if price > 0:
                            ...
                            if mc > 0:
                                ...
                                # Пробуем pools, потом CMC
                                mc2, is_fdv2 = await _fetch_mcap_from_pools(ca)
                                if mc2 > 0:
                                    return price, symbol, mc2, is_fdv2, liq, image
                                mc3, fdv3, is_fdv3 = await _fetch_mcap_from_cmc(symbol)
                                return price, symbol, mc3 or fdv3, is_fdv3, liq, image
                    elif r.status == 429:
...
async def get_token_history(ca, timeframe="7D"):
    ...
    resolution, days = tf_map.get(timeframe, ("4h", "7"))

    # DexScreener — работает для TON токенов без лимитов
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    if pairs:
                        ...
                        chain = pair.get("chainId", "ton")
                        if pair_address:
                            async with s.get(
                                f"https://api.dexscreener.com/latest/dex/chart/{chain}/{pair_address}?res={resolution}&cb={days}",
                                timeout=aiohttp.ClientTimeout(total=10)
                            ) as r2:
                                if r2.status == 200:
                                    cdata = await r2.json()
                                    ...
                                    if candles and len(candles) >= 2:
                                        # Возвращаем как [[timestamp_ms, close], ...]
                                        history = [[c[0], float(c[4])] for c in candles if len(c) >= 5]
                                        if len(history) >= 2:
                                            logger.info(f"DexScreener history: {len(history)} candles for {ca[:8]}")
                                            return history
    except Exception as e:
        logger.warning(f"DexScreener history: {e}")

    # Fallback — GeckoTerminal
    gt_tf = {"1H": "minute/1", "6H": "hour/1", "24H": "hour/1", "7D": "hour/4", "30D": "day/1"}
    gt_limit = {"1H": 60, "6H": 360, "24H": 24, "7D": 42, "30D": 30}
    path = gt_tf.get(timeframe, "hour/4")
    limit = gt_limit.get(timeframe, 42)
    for attempt in range(2):
        try:
            async with aiohttp.ClientSession() as s:
                async with s.get(
                    f"https://api.geckoterminal.com/api/v2/networks/ton/tokens/{ca}/ohlcv/{path}?limit={limit}",
                    timeout=aiohttp.ClientTimeout(total=10),
                    headers={"Accept": "application/json"}
                ) as r:
                    if r.status == 200:
                        ...
                        if candles and len(candles) >= 2:
                            ...
                            logger.info(f"GeckoTerminal history: {len(history)} candles")
                            return history
                    elif r.status == 429:
                        await asyncio.sleep(2)
        except Exception as e:
            logger.warning(f"GT history attempt {attempt+1}: {e}")
            if attempt < 1:
...
async def sell_coin_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    if ca:
        price, _, cur_mcap, is_fdv, _, image_url = await fetch_token_data(ca)
    else:
