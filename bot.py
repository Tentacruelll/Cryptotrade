...
async def get_token_history(ca, timeframe="7D"):
    ...
    resolution, limit = tf_map.get(timeframe, ("h4", 42))

    # 1. DexScreener — ищем пару и берём OHLCV
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    if ton_pairs:
                        ...
                        if pair_addr:
                            async with s.get(
                                f"https://api.dexscreener.com/latest/dex/ohlcv/{chain}/{pair_addr}?res={resolution}&limit={limit}",
                                timeout=aiohttp.ClientTimeout(total=10)
                            ) as r2:
                                if r2.status == 200:
                                    cdata = await r2.json()
                                    ohlcv = cdata.get("ohlcv") or []
                                    if len(ohlcv) >= 2:
                                        history = [[c[0], float(c[4])] for c in ohlcv if len(c) >= 5]
                                        if len(history) >= 2:
                                            logger.info(f"DexScreener: {len(history)} candles")
                                            return history
    except Exception as e:
        logger.warning(f"DexScreener history: {e}")

    # 2. GeckoTerminal fallback
    gt_path = {"1H": "minute/1", "6H": "hour/1", "24H": "hour/1", "7D": "hour/4", "30D": "day/1"}
    gt_limit = {"1H": 60, "6H": 360, "24H": 24, "7D": 42, "30D": 30}
    path = gt_path.get(timeframe, "hour/4")
    lim = gt_limit.get(timeframe, 42)
    for attempt in range(2):
        try:
            async with aiohttp.ClientSession() as s:
                async with s.get(
                    f"https://api.geckoterminal.com/api/v2/networks/ton/tokens/{ca}/ohlcv/{path}?limit={lim}",
                    timeout=aiohttp.ClientTimeout(total=10),
                    headers={"Accept": "application/json"}
                ) as r:
                    if r.status == 200:
                        ...
                        if len(candles) >= 2:
                            ...
                            logger.info(f"GeckoTerminal: {len(history)} candles")
                            return history
                    elif r.status == 429:
                        await asyncio.sleep(2)
        except Exception as e:
            logger.warning(f"GT history {attempt+1}: {e}")
            if attempt < 1:
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    async def fetch_one(row):
        ...
        if ca:
            try:
                price, _, cur_mcap, is_fdv, _, _ = await fetch_token_data(ca)
                if not price:
                    price = avg_price
            except:
