...
def init_db():
    ...
    # Индекс для быстрого поиска по CA
    try:
        c.execute("CREATE INDEX IF NOT EXISTS idx_portfolio_ca ON portfolio(user_id, ca)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_portfolio_sym ON portfolio(user_id, symbol)")
    except: pass
...
def update_portfolio(user_id, symbol, amount_delta, price, ca="", mcap=0):
    conn = sqlite3.connect("trading.db"); c = conn.cursor()
    # Ищем по CA если есть, иначе по символу
    if ca:
        c.execute("SELECT amount,avg_buy_price,avg_mcap FROM portfolio WHERE user_id=? AND ca=?", (user_id, ca))
    else:
    ...
    if row:
        ...
        if new_amt <= 0.000001:
            if ca:
                c.execute("DELETE FROM portfolio WHERE user_id=? AND ca=?", (user_id, ca))
            else:
                c.execute("DELETE FROM portfolio WHERE user_id=? AND symbol=?", (user_id, symbol))
        else:
            if ca:
                c.execute("UPDATE portfolio SET amount=?,avg_buy_price=?,avg_mcap=?,symbol=? WHERE user_id=? AND ca=?",
        ...
        if amount_delta > 0:
            try:
                ...
                logger.error(f"Portfolio insert error: {e}")
    conn.commit(); conn.close()
