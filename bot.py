...
TON_CA_PATTERN = re.compile(r"(EQ[A-Za-z0-9_-]{46,64}|UQ[A-Za-z0-9_-]{46,64}|0:[A-Fa-f0-9]{64})")[A-Za-z0-9_\-]{46,64}\b')
...
# ─── TON PRICE CACHE ─────────────────────────────────────────────────────────
_ton_cache = {"price": None, "updated": 0}

async def get_ton_price():
    now = asyncio.get_event_loop().time()
    if now - _ton_cache["updated"] < 60:
        return _ton_cache["price"] if _ton_cache["price"] is not None else 0
    try:
...
async def fetch_token_data(ca: str):
    ...
    _token_cache[ca] = {"data": (0.0, ca[:8].upper(), 0.0, True, 0.0, ""), "updated": asyncio.get_event_loop().time()}
