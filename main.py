...
def back_keyboard(target="back_main"):
    ...
    kb.add(InlineKeyboardButton("◀️ Back", callback_data=target))
    return kb
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💎 Balance:  <b>{format_gram(user[2])} GRAM</b>\n"
        f"≈ ${user[2]*ton_price:,.2f}\n\n"
        f"📈 TON Price:  <b>${ton_price:.4f}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🚀 Ready to trade?\n"
        f"Send any TON contract address to start a demo trade.\n\n"
        f"⚙️ /setbalance — customize your demo balance",
...
async def share_pnl(callback: types.CallbackQuery):
    ...
    if card:
        caption = (
            ...
            f"<b>Bought: {format_gram(ton_start)} 💎  ·  Sold: {format_gram(ton_end)} 💎</b>\n\n"
            f"🐙 OCTOtrade"
        )
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    for res in results:
        ...
        lines.append(
            ...
            f"   {format_gram(invested_gram)} 💎GRAM → {format_gram(value)}💎GRAM   {'+' if pnl_gram>=0 else ''}{pnl_gram:.2f}💎\n"
...
async def show_history(callback: types.CallbackQuery):
    ...
    for day_str, cnt in days:
        ...
        lines.append(f"<b>{label}</b>\n   {cnt} trade{'s' if cnt != 1 else ''}")
        kb.add(InlineKeyboardButton(f"{label}  ·  {cnt} trades", callback_data=f"hday_{day_str}"))
    kb.add(InlineKeyboardButton("◀️ Back", callback_data="back_main"))
...
async def show_history_day(callback: types.CallbackQuery):
    ...
    if not trades:
        await edit_or_answer(callback.message,
            f"📊 <b>{label}</b>\n\n<i>No trades.</i>",
            reply_markup=back_keyboard("history"), parse_mode="HTML"); return
...
async def show_performance(callback: types.CallbackQuery):
    ...
    if not stats or stats["total_trades"] == 0:
        await edit_or_answer(callback.message,
            "📈 <b>Performance</b>\n\n<i>No trades yet. Start trading to see your stats!</i>",
            reply_markup=back_keyboard(), parse_mode="HTML"); return

    text = (
        f"📈 <b>Performance</b>\n\n"
        f"Trades:  <b>{stats['total_trades']}</b>\n"
        f"Win rate:  <b>{stats['win_rate']:.0f}%</b>\n"
        f"Volume:  <b>{format_gram(stats['volume'])} 💎</b>\n"
        f"Best trade:  <b>{stats['best_trade']:+.0f}%</b>\n"
        f"Avg hold:  <b>{_fmt_hold_time(stats['avg_hold_min'])}</b>"
    )
    await edit_or_answer(callback.message, text, reply_markup=back_keyboard(), parse_mode="HTML")
