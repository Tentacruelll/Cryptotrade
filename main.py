...
TRANSLATIONS = {
    ...
    "price_load_failed": {"en": "⏳ Price unavailable — try Refresh", "ru": "⏳ Цена недоступна — нажми Обновить"},
...
async def sell_coin_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not price:
        # Цена не загрузилась — НЕ позволяем продать "по старой цене" (это эксплойт)
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton(t("btn_refresh", lang), callback_data=f"sellcoin_{symbol}"))
        kb.add(InlineKeyboardButton(t("btn_back", lang), callback_data="back_main"))
        await edit_or_answer(callback.message, t("price_load_failed", lang), reply_markup=kb, parse_mode="HTML")
        return
    ton_price = await get_ton_price()
    price_in_gram = price / ton_price
    avg_price_gram = avg_price / ton_price
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    async def fetch_one(row):
        ...
        if ca:
            try:
                ...
                if not price:
                    # Данные не пришли — помечаем как "не загрузилось", НЕ подставляем старую цену
                    return sym, amount, avg_price, ca, avg_mcap, None, None
                if fetched_sym and len(fetched_sym) <= 12 and not re.match(r'^[A-Z0-9]{8,}$', fetched_sym):
                    sym = fetched_sym
                elif fetched_sym and fetched_sym != ca[:8].upper():
                    sym = fetched_sym
            except:
                # Таймаут/ошибка — помечаем как "не загрузилось", НЕ подставляем старую цену
                return sym, amount, avg_price, ca, avg_mcap, None, None
        else:
    ...
    for res in results:
        ...
        if price is None:
            # Не удалось получить актуальную цену — показываем честно, не подставляем фейковый 0%
            invested_gram = amount * (avg_price / ton_price) if ton_price > 0 else 0
            lines.append(
                f"⚪ {dex}\n"
                f"   {t('price_load_failed', lang)}\n"
                f"   {format_gram(invested_gram)} 💎GRAM"
            )
            continue
