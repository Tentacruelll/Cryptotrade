...
def generate_pnl_card(symbol, pnl_pct, pnl_gram, mcap_buy, mcap_sell,
    ...
    try:
        ...
        ax.text(0.5, 6.0, f"{arrow} {pnl_pct:+.3f}%", color=accent,
        ...
        ax.text(3.2, 4.5, f"{bought_str}", color="white", fontsize=13,
                fontweight="bold", va="center", zorder=5)

        ax.text(0.5, 3.6, "Sold", color="#888888", fontsize=11, va="center", zorder=5)
        ax.text(3.2, 3.6, f"{sold_str}", color=accent, fontsize=13,
                fontweight="bold", va="center", zorder=5)
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    for res in results:
        ...
        lines.append(
            ...
            f"   💎 {format_gram(invested_gram)} —→ 💎 {format_gram(value)}  <i>(≈ ${value*ton_price:,.2f})</i>  ({'+' if pnl_gram>=0 else ''}{format_gram(pnl_gram)} GRAM)"
        )
...
async def show_history(callback: types.CallbackQuery):
    ...
    for sym, action, amount, price, price_buy, total, pnl_pct, mcap_buy, mcap_sell, ts in trades:
        if action == "BUY":
            ...
            lines.append(
                f"🔴 {ts}  ·  <b>{sym}</b>\n"
                f"   💎 {format_gram(total)}  ·  {format_mcap(mcap_sell, False)}"
            )
    await edit_or_answer(callback.message, "\n".join(lines), reply_markup=back_keyboard(), parse_mode="HTML")
