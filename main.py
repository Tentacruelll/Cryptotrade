...
def init_db():
    ...
    c.execute(f"""CREATE TABLE IF NOT EXISTS portfolio (
    ...
    c.execute(f"""CREATE TABLE IF NOT EXISTS calls (
        ...
        timestamp TEXT)""")
    try: c.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_calls_unique ON calls(chat_id, ca)")
    except: pass
    for col in [
    ...
    try:
        c.execute("CREATE INDEX IF NOT EXISTS idx_portfolio_ca ON portfolio(user_id, ca)")
    except: pass
                            ...
                            if tf == "hour" and aggregate == "4":
                                ...
                                async with s.get(url2, timeout=aiohttp.ClientTimeout(total=15),
                                    ...
                                    if r2.status == 200:
                                        ...
                                        if len(candles2) >= 2:
                                            history = [[c[0] * 1000, float(c[4])] for c in candles2]
...
def _get_random_bg(is_win: bool):
    ...
    folder = "pnl_backgrounds/positive" if is_win else "pnl_backgrounds/negative"
    try:
...
def generate_pnl_card(symbol, pnl_pct, pnl_gram, mcap_buy, mcap_sell,
    ...
    try:
        ...
        bg_fallback = "#0a1a10" if is_win else "#1a0a0a"

        fig, ax = plt.subplots(figsize=(8, 4.5))
        ax.set_xlim(0, 10); ax.set_ylim(0, 10); ax.axis("off")

        # Фоновое изображение из папки
        bg_arr = _get_random_bg(is_win)
        if bg_arr is not None:
            ax.imshow(bg_arr, extent=[0, 10, 0, 10], aspect="auto", zorder=0)
            fig.patch.set_facecolor(bg_fallback)
            ax.set_facecolor("none")
            # Полупрозрачный оверлей для читаемости текста
            overlay = mpatches.FancyBboxPatch((0, 0), 10, 10,
                boxstyle="square,pad=0", facecolor="black", alpha=0.45, zorder=1)
        ...
        for lw, alpha in [(18, 0.04), (10, 0.08), (4, 0.18)]:
            rect = mpatches.FancyBboxPatch((0.1, 0.1), 9.8, 9.8,
                ...
                edgecolor=accent, facecolor="none", alpha=alpha, zorder=2)
            ax.add_patch(rect)
        ...
        if image_arr is not None:
            try:
                ...
                ab = AnnotationBbox(im, (1.1, 8.2), frameon=False, zorder=3)
                ax.add_artist(ab)
            except: pass

        # Текст
        ax.text(2.2, 8.3, symbol, color="white", fontsize=20, fontweight="bold", va="center", zorder=4)
        ...
        if mcap_buy > 0 and mcap_sell > 0:
            ...
            ax.text(5, 5.0, f"{x_mult:.2f}x", color=accent, fontsize=18,
                    fontweight="bold", ha="center", alpha=0.85, zorder=4)
        gram_sign = "+" if pnl_gram >= 0 else ""
        ax.text(5, 3.9, f"{gram_sign}{format_gram(pnl_gram)} 💎 GRAM",
                color="#ffffff", fontsize=12, ha="center", zorder=4)
        if mcap_buy > 0 and mcap_sell > 0:
            mc_pct = (mcap_sell - mcap_buy) / mcap_buy * 100
            ax.text(5, 2.9,
                    f"MCap  {format_mcap_val(mcap_buy)}  →  {format_mcap_val(mcap_sell)}  ({mc_pct:+.0f}%)",
                    color="#cccccc", fontsize=9, ha="center", zorder=4)
        ax.text(3.0, 1.9, f"Entry: {format_price(entry_price)}", color="#cccccc", fontsize=9, ha="center", zorder=4)
        ax.text(7.0, 1.9, f"Exit: {format_price(exit_price)}", color="#cccccc", fontsize=9, ha="center", zorder=4)
        ax.text(5, 0.8, "🐙 OCTOtrade  ·  Simulator", color="#aaaaaa", fontsize=8, ha="center", zorder=4)
...
async def send_token_info(chat_id, ca, state: FSMContext, edit_msg=None, timeframe="7D"):
    ...
    if history_365 and len(history_365) >= 2 and price > 0 and mcap > 0:
        try:
            ...
            max_price = max(c[1] for c in history_365)
            ath_fdv = max_price * supply
            ath_fdv_str = f"\n🏔 ATH FDV:  <b>{format_mcap_val(ath_fdv)}</b>"
        except: pass


    # Пересобираем caption с актуальным ath_fdv_str
    caption = (
        f"🪙 <b>{symbol}</b>\n\n"
        f"💵 Price:  <b>{price_usd_str}</b>  ·  <b>{price_gram_str} 💎</b>\n"
        f"📊 FDV:  <b>{mcap_str}</b>\n"
...
async def cmd_cruel(message: types.Message):
    ...
    rows = get_top_callers(chat_id, limit=10)
    if not rows:
    ...
    for i, (username, ca, symbol, price_at_call, mcap_at_call, timestamp) in enumerate(rows, 1):
        # Берём текущую цену из кэша если есть
        try:
...
async def quick_amount(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not price:
        await callback.answer("❌ Price unavailable — resend CA", show_alert=True)
        return
...
async def sell_menu(callback: types.CallbackQuery):
    ...
    kb.add(InlineKeyboardButton("◀️ Back", callback_data="back_main"))
    await edit_or_answer(callback.message, "🔴 <b>Sell Position</b>\n\nSelect:", reply_markup=kb, parse_mode="HTML")
...
async def sell_coin_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    held, avg_price, ca, avg_mcap = get_position(callback.from_user.id, symbol)
    if held <= 0:
        await edit_or_answer(callback.message, "❌ Position not found.", reply_markup=back_keyboard())
...
async def confirm_sell(callback: types.CallbackQuery, state: FSMContext):
    ...
    await edit_or_answer(callback.message, "⏳ <b>Processing sell...</b>", parse_mode="HTML")
    await asyncio.sleep(2)
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    if not holdings:
        await edit_or_answer(callback.message,
            f"💼 <b>Portfolio</b>\n\n"
            f"💎 Balance:  <b>{format_gram(balance)} GRAM</b>  <i>(≈ ${balance*ton_price:,.2f})</i>\n\n"
