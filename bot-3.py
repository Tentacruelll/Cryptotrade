...
def update_portfolio(user_id, symbol, amount_delta, price, ca="", mcap=0):
    ...
    row = c.fetchone()
    if row:
        old_amount, old_avg, old_mcap = row
        new_amount = old_amount + amount_delta
        if amount_delta > 0 and new_amount > 0:
            new_avg = (old_amount*old_avg + amount_delta*price) / new_amount
            new_mcap = (old_amount*old_mcap + amount_delta*mcap) / new_amount if mcap > 0 else old_mcap
        else:
        ...
        if new_amount <= 0.000001:
            ...
            c.execute("UPDATE portfolio SET amount=?,avg_buy_price=?,avg_mcap=? WHERE user_id=? AND symbol=?",
                      (new_amount, new_avg, new_mcap, user_id, symbol))
    else:
        if amount_delta > 0:
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get("https://api.coingecko.com/api/v3/simple/price?ids=the-open-network&vs_currencies=usd",
                ...
                if r.status == 200:
                    data = await r.json()
                    return float(data.get("the-open-network", {}).get("usd", 5.0))
    except: pass
    return 5.0

async def fetch_token_data(ca: str):
    """Returns (price_usd, symbol, fdv, liquidity, image_url, lp_burned_pct, renounced)"""
    # 1. GeckoTerminal — основной источник
                ...
                async with s.get(
                    ...
                    if r.status == 200:
                        ...
                        if price > 0:
                            ...
                            return price, symbol, fdv, liq, image, 0.0, False
                    elif r.status == 429:
                ...
                async with s.get(f"https://api.ston.fi/v1/assets/{ca}",
                    ...
                    if r.status == 200:
                        ...
                        if price > 0:
                            ...
                            return price, symbol, 0.0, 0.0, "", 0.0, False
        except Exception as e:
            logger.warning(f"STON attempt {attempt+1}: {e}")
            await asyncio.sleep(1)

    # 3. DeDust via GeckoTerminal pools
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    if pools:
                        ...
                        if price > 0:
                            return price, ca[:8].upper(), 0.0, 0.0, "", 0.0, False
    except Exception as e:
    ...
    return 0.0, ca[:8].upper(), 0.0, 0.0, "", 0.0, False

async def get_ton_token_history(ca, days=7):
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                f"https://api.geckoterminal.com/api/v2/networks/ton/tokens/{ca}/ohlcv/day?limit={days}",
                timeout=aiohttp.ClientTimeout(total=10),
                headers={"Accept": "application/json"}
            ) as r:
                if r.status == 200:
...
def format_gram(v):
    """Smart GRAM formatting — no useless trailing zeros"""
    if v == 0: return "0"
    if v >= 1:
        # e.g. 10.25, 10.1, 1000 — up to 2 decimal places, strip zeros
        s = f"{v:.2f}".rstrip('0').rstrip('.')
        return s
    # Small values like 0.0045
...
def mcap_multiplier(buy_mcap, cur_mcap):
    if not buy_mcap or buy_mcap <= 0: return None
    return cur_mcap / buy_mcap
...
def apply_slippage(amount):
    slip = random.uniform(0.005, 0.025)
    return amount * (1 - slip), slip * 100
...
def generate_chart(symbol, history, image_arr=None, entry_price=None, entry_mcap=None, fdv=None):
    if not history:
        return None
    ...
    if entry_price and entry_price > 0:
        ...
        ax.text(timestamps[0], entry_price, "  Entry", color="#ffd740", fontsize=8, va="bottom", alpha=0.9)
    ...
    arrow = "▲" if is_up else "▼"
    title_color = color
    title = f"{symbol}   {arrow} {change:+.1f}%"
    if fdv and fdv > 0:
        title += f"   MCap {format_mcap(fdv)}"
    ax.set_title(title, color=title_color, fontsize=13, fontweight="bold", pad=14)
...
def generate_pnl_card(symbol, pnl_pct, pnl_gram, mcap_buy, mcap_sell, entry_price, exit_price, image_arr=None):
    is_win = pnl_pct >= 0
    ...
    pnl_text = f"{arrow}  {pnl_pct:+.2f}%"
    ax.text(5, 6.0, pnl_text, color=accent, fontsize=34, fontweight="bold",
            ha="center", va="center")

    # GRAM pnl
    gram_sign = "+" if pnl_gram >= 0 else ""
    ax.text(5, 4.5, f"{gram_sign}{format_gram(pnl_gram)} 💎 GRAM", color="#cccccc",
            fontsize=13, ha="center", va="center")

    # MCap info
    if mcap_buy > 0 and mcap_sell > 0:
        mult = mcap_sell / mcap_buy
        ax.text(5, 3.2, f"MCap  {format_mcap(mcap_buy)}  →  {format_mcap(mcap_sell)}  ({mult:.2f}x)",
                color="#888899", fontsize=10, ha="center", va="center")

    # Entry / Exit
    ax.text(3.0, 2.0, f"Entry: {format_price(entry_price)}", color="#666688",
            fontsize=9, ha="center")
    ax.text(7.0, 2.0, f"Exit: {format_price(exit_price)}", color="#666688",
            fontsize=9, ha="center")

    # Watermark
    ax.text(5, 0.7, "Tenta Trader · Demo", color="#333355", fontsize=8, ha="center")
...
def token_keyboard(safe_sym, ca):
    ...
    kb.add(InlineKeyboardButton("🔄 Refresh", callback_data=f"refresh_{ca}"))
    kb.add(InlineKeyboardButton("◀️ Back", callback_data="back_main"))
    return kb
...
async def cmd_start(message: types.Message):
    ...
    balance_usd = user[2] * ton_price
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("💎 Portfolio", callback_data="portfolio"),
           InlineKeyboardButton("📊 History", callback_data="history"))
    kb.add(InlineKeyboardButton("🔴 Sell", callback_data="sell"),
           InlineKeyboardButton("💎 Balance", callback_data="balance_menu"))
    kb.add(InlineKeyboardButton("🔄 Reset", callback_data="reset_confirm"))
    kb.add(InlineKeyboardButton("📢 Channel", url=CHANNEL_URL))
    await message.answer(
        f"👋 Welcome back, <b>{message.from_user.first_name}</b>\n\n"
        f"<b>Tenta Trader</b> — TON demo trading terminal\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💎 Balance:  <b>{format_gram(user[2])} GRAM</b>  <i>(≈ ${balance_usd:,.2f})</i>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"Drop any TON contract address in chat to get started.\n\n"
        f"<i>/setbalance — set a custom starting balance</i>",
...
async def cmd_setbalance(message: types.Message):
    ...
    await message.answer(f"✅ Balance updated — <b>{amount:,.4f} 💎 GRAM</b>", reply_markup=main_keyboard(), parse_mode="HTML")
...
async def back_main(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"🏠 <b>Main Menu</b>\n\n"
        f"💎 <b>{format_gram(user[2])} GRAM</b>  <i>(≈ ${balance_usd:,.2f})</i>\n\n"
...
async def send_token_info(chat_id, ca, state: FSMContext, edit_msg=None):
    price, symbol, fdv, liq, image_url, _, _ = await fetch_token_data(ca)
    if price == 0:
        text = (
            "❌ <b>Token not found</b>\n\n"
            "Not listed on DeDust / STON.fi yet.\n"
            "Try again once liquidity is added."
        )
        if edit_msg:
    ...
    await state.finish()
    await state.update_data(ca=ca, symbol=symbol, price=price, fdv=fdv,
                            safe_sym=safe_sym, image_url=image_url, liq=liq)
    await CAState.entering_ca_amount.set()

    liq_str = format_mcap(liq) if liq > 0 else "N/A"
    caption = (
        f"🪙 <b>{symbol}</b>\n\n"
        f"💵 Price:  <b>{format_price(price)}</b>  •  <b>{price_in_gram:.6f} 💎</b>\n"
        f"📊 MCap:  <b>{format_mcap(fdv)}</b>\n"
        f"💧 Liquidity:  <b>{liq_str}</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💎 Your balance:  <b>{format_gram(balance)} GRAM</b>  <i>(≈ ${balance_usd:,.2f})</i>\n"
        f"📦 Max buy:  <b>{balance/price_in_gram:,.2f} {symbol}</b>\n\n"
        f"🔗 <a href='{dex_link(ca)}'>DexScreener</a>\n\n"
        f"Enter amount in <b>💎 GRAM</b> or tap:"
    )

    # Попытка загрузить картинку и сгенерировать график
    image_arr = load_token_image(image_url) if image_url else None
    history = await get_ton_token_history(ca, days=7)
    chart_buf = generate_chart(symbol, history, image_arr=image_arr, fdv=fdv) if history else None
...
async def quick_amount(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "ca":
        gram_amount = balance * pct / 100
        if gram_amount <= 0: await callback.answer("❌ Insufficient balance", show_alert=True); return
        coin_raw = gram_amount / price_in_gram
        coin_after_slip, slip_pct = apply_slippage(coin_raw)
        await state.update_data(gram_amount=gram_amount, coin_amount=coin_after_slip,
                                slip_pct=slip_pct, mcap=fdv)
        ...
        await callback.message.edit_text(
            ...
            f"🪙 {symbol}:  <b>{coin_after_slip:,.4f}</b>\n"
            f"💵 Price:  {format_price(price)}\n"
            f"💎 Cost:  <b>{format_gram(gram_amount)} GRAM</b>\n"
            f"📊 MCap:  {format_mcap(fdv)}\n"
            f"⚡️ Slippage:  -{slip_pct:.2f}%",
            reply_markup=kb, parse_mode="HTML"
        )
    elif action == "sell":
        held, avg_price, _, avg_mcap = get_position(callback.from_user.id, symbol)
        if held <= 0: await callback.answer("❌ No position", show_alert=True); return
        coin_amount = held * pct / 100
        gram_received = coin_amount * price_in_gram
        pnl_pct = (price - avg_price) / avg_price * 100 if avg_price > 0 else 0
        pnl_gram = coin_amount * (price_in_gram - avg_price / ton_price * ton_price / ton_price)
        pnl_gram = gram_received - coin_amount * (avg_price / ton_price)
        ...
        await state.update_data(coin_amount=coin_amount, gram_received=gram_received,
                                ...
                                avg_price=avg_price, avg_mcap=avg_mcap, cur_mcap=fdv or avg_mcap)
        ...
        await callback.message.edit_text(
            ...
            f"🪙 {symbol}:  <b>{coin_amount:,.4f}</b>\n"
            f"💵 Price:  {format_price(price)}\n"
            f"💎 Receive:  <b>{format_gram(gram_received)} GRAM</b>\n"
            f"{'🟢' if pnl_pct>=0 else '🔴'} P&L:  <b>{pnl_pct:+.2f}% {arrow}</b>\n"
            f"{pnl_bar(pnl_pct)}",
            reply_markup=kb, parse_mode="HTML"
        )

# ─── CA AMOUNT (text input) ───────────────────────────────────────────────────

@dp.message_handler(state=CAState.entering_ca_amount)
async def ca_amount_entered(message: types.Message, state: FSMContext):
    data = await state.get_data()
    symbol=data["symbol"]; price=data["price"]; fdv=data.get("fdv",0)
    user = get_user(message.from_user.id); balance = user[2]
    ton_price = await get_ton_price()
    price_in_gram = price / ton_price
    text_input = message.text.strip().lower()
    try:
        gram_amount = balance if text_input == "all" else float(text_input)
        if gram_amount <= 0: raise ValueError
        if gram_amount > balance:
            await message.answer(f"❌ Insufficient balance. You have {format_gram(balance)} GRAM"); return
        coin_raw = gram_amount / price_in_gram
        coin_after_slip, slip_pct = apply_slippage(coin_raw)
    except:
...
async def confirm_ca_trade(callback: types.CallbackQuery, state: FSMContext):
    ...
    fdv=data.get("mcap", 0); slip_pct=data.get("slip_pct", 0)
    ...
    update_portfolio(user_id, symbol, coin_amount, price, ca, fdv)
    save_trade(user_id, symbol, "BUY", coin_amount, price, price, gram_amount, 0.0, fdv, 0)
    await state.finish()
    ...
    await callback.message.edit_text(
        f"✅ <b>Swap Filled</b>\n\n"
        f"🪙 Received:  <b>{coin_amount:,.4f} {symbol}</b>\n"
        f"💎 Spent:  <b>{format_gram(gram_amount)} GRAM</b>\n"
        f"💵 Entry price:  {format_price(price)}\n"
        f"📊 MCap at entry:  {format_mcap(fdv)}\n"
        f"⚡️ Slippage applied:  -{slip_pct:.2f}%\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
...
async def sell_menu(callback: types.CallbackQuery):
    ...
    if not holdings:
        await callback.message.edit_text(
            "📭 <b>No open positions</b>\n\nDrop a CA in chat to open a trade.",
...
async def sell_coin_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    price, _, fdv, _, _, _, _ = await fetch_token_data(ca) if ca else (avg_price, symbol, 0, 0, "", 0, False)
    if not price: price = avg_price
    ton_price = await get_ton_price()
    price_in_gram = price / ton_price
    ...
    mult_str = ""
    if avg_mcap > 0 and fdv > 0:
        mult = fdv / avg_mcap
        mult_str = f"\n📊 MCap:  {format_mcap(avg_mcap)} → {format_mcap(fdv)}  ({mult:.2f}x)"

    await state.update_data(symbol=symbol, price=price, ca=ca, safe_sym=safe_sym,
                            held=held, avg_price=avg_price, avg_mcap=avg_mcap, cur_mcap=fdv)
    await SellState.entering_amount.set()

    await callback.message.edit_text(
        f"🔴 <b>Sell {symbol}</b>\n\n"
        f"💵 Price:  <b>{format_price(price)}</b>\n"
        f"📦 Held:  <b>{held:,.4f} {symbol}</b>\n"
        f"📈 Entry:  {format_price(avg_price)}{mult_str}\n"
        f"{'🟢' if pnl_pct>=0 else '🔴'} P&L:  <b>{pnl_pct:+.2f}% {arrow}</b>\n"
        f"{pnl_bar(pnl_pct)}\n\n"
...
async def sell_amount_entered(message: types.Message, state: FSMContext):
    ...
    arrow = pnl_arrow(pnl_pct)
    await state.update_data(coin_amount=coin_amount, gram_received=gram_received,
                            pnl_pct=pnl_pct, pnl_gram=pnl_gram, avg_mcap=avg_mcap, cur_mcap=cur_mcap)
    await SellState.confirming.set()
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("✅ Confirm Sell", callback_data="confirm_sell"),
           InlineKeyboardButton("❌ Cancel", callback_data="back_main"))
...
async def confirm_sell(callback: types.CallbackQuery, state: FSMContext):
    ...
    avg_price=data["avg_price"]; avg_mcap=data.get("avg_mcap",0); cur_mcap=data.get("cur_mcap",0)
    ca=data.get("ca",""); image_url=data.get("image_url","")
    user_id = callback.from_user.id
    ...
    save_trade(user_id, symbol, "SELL", coin_amount, price, avg_price, gram_received, pnl_pct, avg_mcap, cur_mcap)
    await state.finish()
    ...
    await callback.message.edit_text(
        ...
        f"🪙 {symbol}:  -{coin_amount:,.4f}\n"
        f"💎 Received:  <b>{format_gram(gram_received)} GRAM</b>\n"
        f"{'🟢' if pnl_pct>=0 else '🔴'} P&L:  <b>{pnl_pct:+.2f}% {arrow}</b>  ({'+' if pnl_gram>=0 else ''}{format_gram(pnl_gram)} 💎)\n"
        f"{pnl_bar(pnl_pct)}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💎 Balance:  <b>{format_gram(user[2])} GRAM</b>  <i>(≈ ${user[2]*ton_price:,.2f})</i>",
        reply_markup=main_keyboard(), parse_mode="HTML"
    ...
    # PnL карточка
    image_arr = load_token_image(image_url) if image_url else None
    pnl_buf = generate_pnl_card(
        symbol, pnl_pct, pnl_gram, avg_mcap, cur_mcap, avg_price, price, image_arr
    )
    if pnl_buf:
        await bot.send_photo(user_id, InputFile(pnl_buf, filename="pnl.png"),
                             caption=f"{'🟢' if pnl_pct>=0 else '🔴'} <b>{symbol}</b>  {pnl_pct:+.2f}% {arrow}",
                             parse_mode="HTML")
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    if not holdings:
        await callback.message.edit_text(
            f"💼 <b>Portfolio</b>\n\n"
            f"💎 Available:  <b>{format_gram(balance)} GRAM</b>  <i>(≈ ${balance_usd:,.2f})</i>\n\n"
            f"<i>No open positions. Drop a CA to trade.</i>",
            reply_markup=back_keyboard(), parse_mode="HTML"); return

    total_value = balance
    lines = [f"💼 <b>Portfolio</b>\n\n💎 Free:  <b>{format_gram(balance)} GRAM</b>  <i>(≈ ${balance_usd:,.2f})</i>\n"]

    for sym, amount, avg_price, ca, avg_mcap in holdings:
        price, _, fdv, _, _, _, _ = await fetch_token_data(ca) if ca else (avg_price, sym, avg_mcap, 0, "", 0, False)
        if not price: price = avg_price
        price_in_gram = price / ton_price
        avg_price_gram = avg_price / ton_price
        ...
        lines.append(
            ...
            f"   📊 {format_mcap(avg_mcap)} → {format_mcap(fdv)}{mult_str}\n"
            f"   {pnl_bar(pnl_pct)} <b>{pnl_pct:+.2f}% {arrow}</b>\n"
            f"   💎 {format_gram(value)} GRAM  ({'+' if pnl_gram>=0 else ''}{pnl_gram:,.4f})"
        )

    pnl_total = (total_value - STARTING_BALANCE) / STARTING_BALANCE * 100
    pnl_abs = total_value - STARTING_BALANCE
    arrow_t = pnl_arrow(pnl_total)
    lines.append(
        f"\n━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Total:  <b>{format_gram(total_value)} 💎 GRAM</b>  <i>(≈ ${total_value*ton_price:,.2f})</i>\n"
        f"{'🟢' if pnl_total>=0 else '🔴'} Overall P&L:  <b>{pnl_total:+.2f}% {arrow_t}</b>  ({'+' if pnl_abs>=0 else ''}{format_gram(pnl_abs)} 💎)\n"
        f"{pnl_bar(pnl_total)}"
    )
...
async def show_history(callback: types.CallbackQuery):
    ...
    if not trades:
        await callback.message.edit_text(
            "📊 <b>Trade History</b>\n\n<i>No trades yet. Drop a CA to get started.</i>",
            reply_markup=back_keyboard(), parse_mode="HTML"); return
    ...
    for sym, action, amount, price, price_buy, total, pnl_pct, mcap_buy, mcap_sell, ts in trades:
        if action == "BUY":
            lines.append(
                f"🟢 {ts}  ·  <b>{sym}</b>\n"
                f"   +{amount:,.4f}  @  {format_price(price)}  ·  {format_gram(total)} 💎\n"
                f"   📊 MCap: {format_mcap(mcap_buy)}"
            )
        else:
            arrow = pnl_arrow(pnl_pct)
            mult_str = ""
            ...
            lines.append(
                f"🔴 {ts}  ·  <b>{sym}</b>\n"
                f"   -{amount:,.4f}  @  {format_price(price)}  ·  {format_gram(total)} 💎\n"
                f"   {'🟢' if pnl_pct>=0 else '🔴'} <b>{pnl_pct:+.2f}% {arrow}</b>{mult_str}\n"
                f"   {pnl_bar(pnl_pct)}"
            )
...
async def balance_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"1 💎 GRAM = ${ton_price:.4f}  (live TON price)",
        reply_markup=kb, parse_mode="HTML"
    )
...
async def setbalance_received(message: types.Message, state: FSMContext):
    ...
    set_balance(message.from_user.id, amount)
    await state.finish()
    await message.answer(f"✅ Balance set —  <b>{amount:,.4f} 💎 GRAM</b>",
                         reply_markup=main_keyboard(), parse_mode="HTML")
...
async def topup_start(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        f"➕ <b>Add GRAM</b>\n\n"
        f"Current: <b>{format_gram(user[2])} GRAM</b>\n"
...
async def topup_received(message: types.Message, state: FSMContext):
    try:
        ...
        if amount > MAX_TOPUP:
            await message.answer(f"❌ Max top-up is {MAX_TOPUP:,.0f} GRAM per operation"); return
    except:
...
async def reset_confirm(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"All positions and history will be wiped.\n"
...
async def unknown_message(message: types.Message):
    await message.answer(
        "🔍 Drop a TON contract address to trade.\n\nOr use the menu 👇",
...
if __name__ == "__main__":
    init_db()
    logger.info("Tenta Trader started.")
