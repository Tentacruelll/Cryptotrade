...
# ─── TON PRICE CACHE ─────────────────────────────────────────────────────────
_ton_cache = {"price": 5.0, "updated": 0}

async def get_ton_price():
    now = asyncio.get_event_loop().time()
    if now - _ton_cache["updated"] < 60:
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                if r.status == 200:
                    data = await r.json()
                    p = float(data.get("the-open-network", {}).get("usd", 5.0))
                    _ton_cache["price"] = p
                    ...
                    if r.status == 200:
                        ...
                        if price > 0:
                            ...
                            if mc > 0:
                                ...
                                mc2, is_fdv = await _fetch_mcap_from_pools(ca)
                                return price, symbol, mc2, is_fdv, liq, image
                    elif r.status == 429:
...
async def show_portfolio(callback: types.CallbackQuery):
    ...
    pnl_total_gram = total_value - STARTING_BALANCE
    pnl_total_pct = pnl_total_gram / STARTING_BALANCE * 100
    arrow_t = pnl_arrow(pnl_total_pct)
    lines.append(
        f"\n━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Total:  <b>{format_gram(total_value)} 💎 GRAM</b>  <i>(≈ ${total_value*ton_price:,.2f})</i>\n"
        f"{'🟢' if pnl_total_pct>=0 else '🔴'} Overall P&L:  <b>{pnl_total_pct:+.2f}% {arrow_t}</b>  ({'+' if pnl_total_gram>=0 else ''}{format_gram(pnl_total_gram)} 💎)"
    )
